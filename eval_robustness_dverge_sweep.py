...
def get_args_local():
    ...
    parser.add_argument('--sweep-param', default='epsilon', type=str, choices=['eta', 'K', 'epsilon', 'g'])
...
def process_dverge(state_dict):
    ...
    for key in state_dict:
        ...
        if 'fc' in key:
            ...
            new_key = key
        new_state_dict['.'.join(new_key.split('.')[1:])] = state_dict[key]
...
def main():
    ...
    ckpt = torch.load(os.path.join(args.outdir, f'epoch_200.pth'))
    M = len(ckpt.keys())
    print('Number of models detected is {}'.format(M))
    print('Loading the models ...')
    models = []
    for m in range(M):
        model = get_architecture('resnet20_dverge', 'cifar10', args.width_mult, args.expansion_factor, args.channel_mult, args.use_stat_layers)
        ckpt_m = ckpt['model{}'.format(m)]
        if m==0:
            mu = ckpt_m['normalizer.mean'].view(3,1,1)
            std = ckpt_m['normalizer.std'].view(3,1,1)
            normalize = lambda X: (X - mu)/std
        state_dict = process_dverge(ckpt_m)
    ...
    args_to_print = {'eta': eta, 'K': args.attack_iters, 'epsilon': epsilon, 'g': args.g}
